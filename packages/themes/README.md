...
基于 Fluent Design 2 的 UnoCSS 主题**适配层**，全部生成自 `@fluere-vue/designs` 的 token 事实源（`data/fluent-tokens.json`），无手写映射。

## 导出

- `presetFluere()` — UnoCSS preset：
...
- 类型：`Theme`、`BrandVariants`、`FluereTokenName`。
...
产物由 `designs` 生成（见 `@fluere-vue/designs` 的 `sync`），本包只负责 re-export。
